...
sub test_alter_table {
   ...
   if ( grep { $_ eq '--definer' } @$cmds ) {
      like($output, qr/CREATE DEFINER = `\w+` TRIGGER/, "definer given") or $fail = 1;
   } else {
      like($output, qr/CREATE TRIGGER/, "definer not given") or $fail = 1;
   }
...
test_alter_table(
    ...
    new_col   => "last_update",
    cmds      => [
        qw(--execute --definer root),
        '--alter', 'ADD COLUMN last_update DATETIME',
    ],
);
# #############################################################################
# Done.
