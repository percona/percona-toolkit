...
if ($EVAL_ERROR) {
    $sb->load_file('source', 't/pt-table-checksum/samples/pt-131.sql');
    $remove_plugin = 1;
}
...
my $cmd ="PTDEBUG=1 $trunk/bin/pt-table-checksum $source_dsn --disable-qrt-plugin 2>&1";
...
like (
    $output,
    qr/Disabling qrt plugin on source server/,
...
# #############################################################################
if ($remove_plugin) {
   $sb->load_file('source', 't/pt-table-checksum/samples/pt-131-wipe.sql');
}
$sb->wipe_clean($dbh);
