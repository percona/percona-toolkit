...
// distinctNodeColors assigns a stable, distinct color per node label for multi-column views.
var distinctNodeColors = []Color{
	BrightCyanText, BrightMagentaText, BrightBlueText, BrightGreenText, BrightYellowText, BrightWhiteText,
}

// NodeHue returns a terminal color for a node identifier (hostname / short name).
func NodeHue(label string) Color {
	if label == "" || label == "-" {
		return WhiteText
	}
	h := fnv.New32a()
	_, _ = h.Write([]byte(label))
	return distinctNodeColors[h.Sum32()%uint32(len(distinctNodeColors))]
}
...
func ColorForState(state string) string {
	...
	case "ROLLBACK", "STARTUP", "STARTUP2", "RECOVERING", "ARBITER":
		return "yellow"
	case "PRIMARY", "SECONDARY":
		return "green"
	case "DOWN", "REMOVED", "UNKNOWN":
		return "red"
	default:
		return ""
	}
