...
// isSharedName returns true for names that are shared across multiple nodes
// and would bridge unrelated identities in the whois tree (e.g. "listen:27017"
// is common to every mongod on that port, "rs:rs0" is common to every member
// of the replica set).
func isSharedName(name string) bool {
	return strings.HasPrefix(name, "listen:") || strings.HasPrefix(name, "rs:")
}
...
type WhoisValue struct {
	Timestamp *time.Time            `json:"-"`          // used internally for sort order; hidden from output
...
var forcedIterationOrder = []string{"nodename", "ip", "_id"}
...
func (n *WhoisNode) filterDBUsingIP() {
	for ip, valueData := range n.Values {
		for hash, ip2 := range db.HashToIP {
			if ip == ip2.Value {
				valueData.AddChildKey(n, "_id", hash, ip2.Timestamp)
			}
		}
		nodenames, ok := db.IPToNodeNames[ip]
		if ok {
			for _, nodename := range nodenames {
				if isSharedName(nodename.Value) {
					continue
				}
