...
func getfirsttime(l LocalTimeline) time.Time {
	for _, event := range l {
		if event.Date != nil && (event.LogCtx.FileType == MongoLogFileType || event.LogCtx.FileType == "") {
...
func getlasttime(l LocalTimeline) time.Time {
	for i := len(l) - 1; i >= 0; i-- {
		if l[i].Date != nil && (l[i].LogCtx.FileType == MongoLogFileType || l[i].LogCtx.FileType == "") {
