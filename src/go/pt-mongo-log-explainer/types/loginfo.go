...
const (
	Info Verbosity = iota
	// DebugContext includes findings that are usually not relevant to show but useful to create the log context (eg: how we found the local address)
	DebugContext
	Debug
)
...
func NewLogInfo(date *Date, displayer LogDisplayer, log string, regex *LogRegex, regexkey string, logCtx LogCtx, filetype string) LogInfo {
	...
	if filetype != MongoLogFileType && filetype != "" {
