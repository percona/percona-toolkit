...
func main() {
	opts := cliOptions{}
	kCtx, _, err := config.Setup(toolname, opts)
	if err != nil {
		log.Fatalf("Cannot get parameters: %q", err)
	}
	...
	if opts.AllDatabases {
		opts.Databases, err = client.ListDatabaseNames(context.TODO(), primitive.D{})
		if err != nil {
	...
	switch kCtx.Command() {
	case "check-unused":
		resp.Unused = findUnused(ctx, client, opts.Databases, opts.Collections)
	case "check-duplicates":
		resp.Duplicated = findDuplicated(ctx, client, opts.Databases, opts.Collections)
	case "check-all":
		resp.Unused = findUnused(ctx, client, opts.Databases, opts.Collections)
		resp.Duplicated = findDuplicated(ctx, client, opts.Databases, opts.Collections)
	default:
	...
	fmt.Println(output(resp, opts.JSON))
}
